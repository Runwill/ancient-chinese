...
def 声母类(index,mode):
    ...
    else: return ['','ц','ც','ძ','sд','s','თ','д','д','พ','б','ბ','ข','g','к','mh','m','nh','n','rh','r','r','hl','л','hŋ','ง','ء','ㅎ','wh','و'][index]
def 韵头类(index,mode):
...
def 改善咽化替换(text: str):
    text = text.replace("бعΡ","бr").replace("ขع","ข").replace("gع","g").replace('عΡ','ع').replace('عი','ი')
    return text
